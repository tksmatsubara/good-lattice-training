...
# Added arguments
parser.add_argument("--nf", default=20000, type=int, help="number of sampling points")
...
X_f, _ = GLT.get_evaluation_points(args.lattice, lb, ub, N_f)
...
atol = 1e-20
gmres_iter = 500
path = f"Schrodinger/output/Single_Dis_GACGD_tol_{tol}_atol_{atol}_gmresiter_{gmres_iter}_lrmax_{lr_max}_lrmin_{lr_min}_PINN_{PINNlayers}_Dis_{Dislayers}_lattice{args.lattice}_Nf{args.nf}"
...
GACGDoptimizer = CGDs.GACGD(x_params=discriminator.parameters(), y_params=PINNGACGD.parameters(), lr_x=lr_max, lr_y=lr_min, eps=1e-8, tol=tol, atol=atol, max_iter=gmres_iter, track_cond=lambda x, y: True)
print(PINNGACGD)
print(discriminator)
...
for i in range(max_iter):
    ...
    if i % 50 == 0:
        ...
        GACGDInfo = GACGDInfo.append({"iter": i, "u error": error_u.item(), "v error": error_v.item(), "L2 error": error_h.item(), "CPINN loss": loss.item(), "PINN loss": PINNloss.item(), "iter_num_sum": iter_num_sum}, ignore_index=True)
    ...
    if i % 200 == 0:
        ...
        torch.save({"PINN_state_dict": PINNGACGD.state_dict(), "Discriminator_state_dict": discriminator.state_dict(), "optimizer_state_dict": GACGDoptimizer.state_dict()}, path + f"/models/state_dict_iter_{i}.pt")
